...
// マーケティング用の3プロンプト(戦略決定/生成/セルフチェック)とは別の、運用最適化専用の
// 単発Claude呼び出し。時間帯別の投稿実績・エンゲージメント集計から、次回以降
// shouldGenerateNowが参照する「時間帯ごとの投稿重み」を算出させる。
export async function analyzePostingTimes(model: string, statsText: string): Promise<AnalyzePostingTimesResult> {
  const prompt = [
    "あなたはSNS運用データの分析担当です。",
    "以下は、洗濯・仕上げ剤ジャンルのXアカウントにおける、投稿時刻(JST、0〜23時)ごとの",
    "投稿件数と平均エンゲージメント率(いいね+リポスト+返信 / インプレッション)の集計です。",
    "",
    statsText,
    "",
    "# タスク",
