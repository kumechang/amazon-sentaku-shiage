...
// 戦略決定・セルフチェックの2ステージはJSON出力必須のため、このモジュールで共通化している。
...
// JSON期待のステージ呼び出し + zod検証。パース/検証に失敗したら「JSONのみ出力」を
// 強調した1回だけの再試行を行い、それでも駄目なら例外を投げてワークフローを失敗させる
// (壊れた投稿データを黙ってDBに残さないため)。
export async function callClaudeJson<T>(model: string, prompt: string, schema: ZodType<T>): Promise<{ raw: string; data: T }> {
